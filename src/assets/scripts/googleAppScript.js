...
function doGet(e) {
  ...
  if (sheetName === "Timeslots") {
    var sheet =
      SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Timeslots")
    ...
    return ContentService.createTextOutput(
      JSON.stringify({ result: "failure", message: "Invalid sheet name" })
...
function doPost(e) {
  ...
  if (!allAvailable) {
    ...
    return ContentService.createTextOutput(
      JSON.stringify({
        result: "failure",
        message: "One or more dates in the range are unavailable",
        data: data,
        values: values,
      })
    ).setMimeType(ContentService.MimeType.JSON)
  ...
  var body =
    "Danke für deine Buchung | Thank you for your booking.<br/>" +
